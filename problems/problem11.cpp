...
int arr[20][20] = {8,2,22,97,38,15,0,4,0,75,4,5,7,78,52,12,5,77,91,8,
...
bool isAlready(int pos, std::vector<loc> locs)
{
	bool already = false;
	std::vector<loc>::iterator it;
	for(it = locs.begin(); it != locs.end(); it++)
	{
		if(pos == it->tag) return true;
	}
...
loc topAdjacent(loc pos, std::vector<loc> locs)
{
	int top = 0;
	loc l;
	...
	if(pos.x > 0 && pos.y > 0)
	{
		if(std::max(arr[pos.y-1][pos.x-1], top) != top)
		{	
			if(!isAlready((pos.y-1)*20+(pos.x-1), locs))
			{
				top = std::max(arr[pos.y-1][pos.x-1], top);
				l.x = pos.x-1;
				l.y = pos.y-1;
				l.tag = (pos.y-1)*20+(pos.x-1);
				l.value = top;
			}
		}
	}
	...
	if(pos.y > 0)
	{
		if(std::max(arr[pos.y-1][pos.x], top) != top)
		{	
			if(!isAlready((pos.y-1)*20+(pos.x), locs))
			{
				top = std::max(arr[pos.y-1][pos.x], top);
				l.x = pos.x;
				l.y = pos.y-1;
				l.tag = (pos.y-1)*20+(pos.x);
				l.value = top;
			}
		}
	}
	...
	if(pos.x > 0)
	{
		if(std::max(arr[pos.y-1][pos.x], top) != top)
		{	
			if(!isAlready((pos.y)*20+(pos.x-1), locs))
			{
				top = std::max(arr[pos.y][pos.x-1], top);
				l.x = pos.x-1;
				l.y = pos.y;
				l.tag = (pos.y)*20+(pos.x-1);
				l.value = top;
			}
		}
	}
	...
	if(pos.x < 19)
	{
		if(std::max(arr[pos.y][pos.x+1], top) != top)
		{	
			if(!isAlready((pos.y)*20+(pos.x+1), locs))
			{
				top = std::max(arr[pos.y][pos.x+1], top);
...
void Problem11::run()
{
	...
	*/
	...
	//method: create greatestAdjacent method that accepts a position and a list of previous positions to exclude to avoid duplication
	...
	for(int y = 0; y < 20; y++)
	{
		...
		for(int x = 0; x < 20; x++)
		{
			...
			std::cout << "Examining: " << temp.value;

			locs.push_back(temp);

			temp = topAdjacent(temp, locs);
			std::cout << " Next Best: " << temp.value;
			locs.push_back(temp);
			...
			std::cout << " Product: " << tempprod << std::endl;
			
			if(tempprod > product) product = tempprod;
	...
	std::cout << "The greatest product of four adjacent numbers in any direction is: " << product << std::endl;
